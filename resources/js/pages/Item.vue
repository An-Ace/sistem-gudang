<template>
  <div class="card">
    <DataTable paginator :rows="5" :value="items" v-model:expandedRows="expandedRows" tableStyle="min-width: 50rem" dataKey="id">
      <template #header>
        <div class="flex justify-end">
          <RouterLink to="/items/add"><Button size="small" severity="info">Tambah Barang +</Button></RouterLink>
        </div>
      </template>
      ...
      <Column header="Actions">
        <template #body="slotProps">
          <RouterLink :to="`/items/${slotProps.data.id}`" class="p-button-text">
          ...
          <Button label="Delete" icon="pi pi-times" class="p-button-text" severity="danger"
            @click="$swal.fire({
              ...
              Api.delete(`items/${slotProps.data.id}`).then((response) => {
                if (true) {
                  this.$swal.fire({
                    title: 'Berhasil',
                  ...
                  items = items.filter(item => item.id !== slotProps.data.id);
                }
              })
            });"
          />
      ...
      <template #expansion="slotProps">
        <div class="p-4">
          <h5>Mutations for Item: {{ slotProps.data.name }}</h5>
          <DataTable :value="slotProps.data.mutations">
            <Column field="user.name" header="Nama User"></Column>
            ...
            <Column header="Action">
              <template #body="slotProps">
                <RouterLink :to="`/items/${slotProps.data.id}/edit`" class="p-button-text">
